...
-}
degree :: (Eq a, Num a) => Poly a -> Integer
degree x = case trimPoly x of
    Poly [0] -> -1
    Poly xs -> genericLength xs - 1
...
monomial :: (Eq a, Num a) => Integer -> a -> Poly a
monomial n x =
    if x == 0 then zero else Poly (reverse (x : genericReplicate n 0))
